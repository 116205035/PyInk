...
def _paint_initial(new_frame: str, stdout: TextIO) -> None:
    ...
    triggered by Phase C1's high-frequency spinner repaints: ``repaint_frame``
    routes height changes through ``_erase_reachable_rows`` (whose budget is
    capped by ``available_rows``) + ``_paint_initial``. When the budget is
    smaller than the old frame, the top old rows stay uncleared, and the
    previous ``_paint_initial`` did a bare ``write(new_frame)`` — leaving
    stale tails whenever a new row was shorter than its predecessor.
    ...
    used by ``_repaint`` (see ``diff.py:_repaint``), so terminal
    compatibility is already validated.
    """
